...
void MainWindow::on_pushButton_clicked()
{
    ...
    QSqlQuery qry;
    qry.prepare("Select * from employer where service=:service");
    qry.bindValue(":service",username);
    qry.exec();
    while (qry.next())

    {
        if ( qry.value(6).toString()==username && qry.value(7).toString()==password &&qry.value(6).toString()=="Administrateur" )
        {
